...
export async function generateMetadata({ params }: ProductProps): Promise<Metadata> {
  ...
  return { title: getDictionary(safeLocale).nav.product };
}
      ...
      {products.length ? <div className="mx-auto grid max-w-7xl gap-x-8 gap-y-16 px-5 pb-24 md:grid-cols-2 md:px-8">
        {products.map((product) => {
          ...
          return <ScrollReveal key={product._id}><article data-reveal className="border-b border-white/15 pb-6">
            <div className="relative aspect-square overflow-hidden bg-[#17191c]">{imageUrl ? <Image src={imageUrl} alt={name} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover transition duration-700 hover:scale-105" /> : <div className="flex h-full items-center justify-center text-center text-[10px] uppercase tracking-[0.3em] text-white/40">[Placeholder product image]</div>}</div>
