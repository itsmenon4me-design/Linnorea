  ...
  return (
    <main className="bg-[var(--color-bg-base)] text-white">
      ...
      <section className="relative mx-5 aspect-[16/9] overflow-hidden bg-[var(--color-bg-elevated)] md:mx-10">
        {validCoverImage ? (
          <Image src={urlFor(validCoverImage).width(3840).height(2160).fit("crop").auto("format").quality(82).url()} alt={title} fill quality={82} priority sizes="100vw" className="object-cover" />
          ...
          {validGallery[0] ? (
            <ScrollReveal as="div">
              <div data-reveal className="relative aspect-[16/9] overflow-hidden bg-[var(--color-bg-elevated)]">
                <Image src={urlFor(validGallery[0]).width(3840).height(2160).fit("crop").auto("format").quality(82).url()} alt={`${title} gallery 1`} fill quality={82} sizes="100vw" className="object-cover" />
              </div>
            </ScrollReveal>
          ) : null}
          {validGallery.length > 1 ? (
            <div className="grid gap-12 md:grid-cols-2 md:gap-x-16 md:gap-y-24">
              {validGallery.slice(1).map((image, index) => {
                const imageUrl = urlFor(image).width(3840).height(2880).fit("crop").auto("format").quality(80).url();
                return (
        ...
        return (
          <ScrollReveal key={`${section.heading ?? section.label ?? "section"}-${index}`} as="section" className="mx-auto grid max-w-[90rem] gap-10 px-5 py-16 md:grid-cols-[0.7fr_1.3fr] md:gap-16 md:px-10 md:py-24">
            ...
            <div data-reveal className="space-y-8">
              ...
              {image ? <div className="relative aspect-[4/3] overflow-hidden bg-[var(--color-bg-elevated)]"><Image src={urlFor(image).width(3840).height(2880).fit("crop").auto("format").quality(80).url()} alt={section.heading || title} fill quality={80} sizes="(min-width: 768px) 60vw, 100vw" className="object-cover" /></div> : null}
            </div>
          </ScrollReveal>
        );
      })}
              ...
              return (
                <Link key={item._id} href={`/project/${item.slug?.current?.trim()}`} className="group block focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white">
                  <div className="relative aspect-[4/3] overflow-hidden bg-[var(--color-bg-elevated)]">
                    {image ? <Image src={urlFor(image).width(3840).height(2880).fit("crop").auto("format").quality(80).url()} alt={itemTitle} fill quality={80} sizes="(min-width: 1024px) 30vw, (min-width: 640px) 50vw, 100vw" className="object-cover" /> : <MediaPlaceholder className="absolute inset-0" />}
                  </div>
